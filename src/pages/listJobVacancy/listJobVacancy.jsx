...
export default function ListJobVacancy() {
  ...
  const handleEdit = () => {
    ...
    const url = `https://final-project-api-alpha.vercel.app/api/jobs/${id}`;
  ...
  const handleSearch = (event) => {
    const newData = filterData.filter(
      (row) =>
        ...
        (salaryMinFilter === '' || row.salary_min >= parseInt(salaryMinFilter)),
    );
    setData(newData);
  };
  ...
  const columns = [
    ...
    },
    {
      name: 'Company Image',
      cell: (row) => (
        <img src={row.company_image_url} alt="joblist" className="w-12 h-12" />
      ),
    },
    {
      name: 'Actions',
